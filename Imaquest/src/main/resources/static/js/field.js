...
// セッションからプレイヤーの初期位置を取得
const playerRow = parseInt(sessionStorage.getItem("playerRow")) || 0;
const playerCol = parseInt(sessionStorage.getItem("playerCol")) || 0;
player.setAttribute("data-row", playerRow);
player.setAttribute("data-col", playerCol);
...
board.addEventListener("click", function (event) {
    ...
    if (isAdjacent && isValidMove(clickedRow, clickedCol)) {
        ...
        // セッションにプレイヤーの位置情報を保存
        sessionStorage.setItem("playerRow", clickedRow);
        sessionStorage.setItem("playerCol", clickedCol);
...
for (let row = 0; row < 20; row++) {
    for (let col = 0; col < 20; col++) {
        ...
        // マスの色を設定
        const savedRow = parseInt(sessionStorage.getItem("playerRow"));
        const savedCol = parseInt(sessionStorage.getItem("playerCol"));
        if (row === savedRow && col === savedCol) {
            square.classList.add("player");
        }
